...
def get_intersect(line1: Tuple[List[int], List[int]], line2: Tuple[List[int], List[int]]) -> Optional[Tuple[float, float]]:
    p1 = line1[0][:2]
    ...
    if norm == 0:
        return None
    px = ((p1[0] * p2[1] - p1[1] * p2[0]) * (p3[0] - p4[0]) - (p1[0] - p2[0]) * (p3[0] * p4[1] - p3[1] * p4[0])) / norm
    py = ((p1[0] * p2[1] - p1[1] * p2[0]) * (p3[1] - p4[1]) - (p1[1] - p2[1]) * (p3[0] * p4[1] - p3[1] * p4[0])) / norm
    if (px - p1[0]) / line1[1][0] < 0 or (px - p3[0]) / line2[1][0] < 0:
        return None
    return (px, py)


def get_intersections_in_area(lines: List[Tuple[List[int], List[int]]], minxy: int, maxxy: int) -> int:
    result = 0
...
def part2_numerically(lines: List[Tuple[List[int], List[int]]]) -> int:
    ...
    x = optimize.least_squares(lambda x: [get_line_line_distance(x, line) for line in lines], x0)
    return sum(x[:3])
...
def part1(input: List[str]) -> int:
    result = get_intersections_in_area(get_lines(input), 200000000000000, 400000000000000)
...
def test_day24_part1(puzzle_input):
    ...
    get_intersect(lines[0], lines[1])
    assert get_intersections_in_area(lines, 7, 27) == 2


def test_day24_part2(puzzle_input):
    lines = get_lines(puzzle_input)
    assert get_line_line_distance(np.array([24, 13, 10, -3, 1, 2]), lines[0]) < 1e-9
